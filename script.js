		...
		$.getJSON( api_url, function(data) {
			...
			$.each(data, function(key,val){
				if(val.iso_code == iso && val.formulation_number){
					...
					if(val.IF){
						docs.push("<li class=\"elementor-icon-list-item\"><a target=\"_blank\" href=\"" + val.IF[0].doc_url + "\" download=\"" + val.formulation_number + "-ifu.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-lightbulb\"></i></span><span class=\"elementor-icon-list-text\">IFU</span></a><small>" + val.IF[0].doc_date + "</small></li>");
					}
					if(val.PSS){
