		...
		$.getJSON( api_url, function(data) {
			...
			$.each(data, function(key,val){
				if(val.iso_code == iso && val.formulation_number){
					...
					if(val.PSS){
						docs.push("<li class=\"elementor-icon-list-item\"><a target=\"_blank\" href=\"" + val.PSS[0].doc_url + "\" download=\"" + val.formulation_number + "-pss.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-flask\"></i></span><span class=\"elementor-icon-list-text notranslate\">PSS</span></a><small>" + val.PSS[0].doc_date + "</small></li>");
					}
					if(val.CR){
						docs.push("<li class=\"elementor-icon-list-item\"><a target=\"_blank\" href=\"" + val.CR[0].doc_url + "\" download=\"" + val.formulation_number + "-crc.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-vial\"></i></span><span class=\"elementor-icon-list-text notranslate\">CRC</span></a><small>" + val.CR[0].doc_date + "</small></li>");
					}
					if(val.SD){
						$.each(val.SD, function(k,v){
							docs.push("<li class=\"elementor-icon-list-item\"><a target=\"_blank\" href=\"" + v.doc_url + "\" download=\"" + val.formulation_number + "-sds.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-triangle\"></i></span><span class=\"elementor-icon-list-text notranslate\">" + v.doc_name + "</span></a><small>" + v.doc_date + "</small></li>");
						});
					}
