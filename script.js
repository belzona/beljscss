...
/* icons for search results */
	...
	$.fn.get_docs = function(iso, prod)
	{
		...
		$.getJSON( api_url + prod, function(data) {
			...
			$.each(data, function(key,val){
				if(val.iso_code == iso && val.formulation_number){
					...
					if(val.IF){
		            	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + val.IF + "\" download=\"" + val.formulation_number + "-ifu.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-lightbulb\"></i></span><span class=\"elementor-icon-list-text\">IFU</span></a></li>");
		            }
		            if(val.PSS){
		            	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + val.PSS + "\" download=\"" + val.formulation_number + "-pss.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-flask\"></i></span><span class=\"elementor-icon-list-text\">PSS</span></a></li>");
		            }
		            if(val.CR){
		            	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + val.CR + "\" download=\"" + val.formulation_number + "-crc.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-vial\"></i></span><span class=\"elementor-icon-list-text\">CRC</span></a></li>");
		            }
		            if(val.SD){
		            	$.each(val.SD, function(k,v){
		                	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + v.doc_url + "\" download=\"" + val.formulation_number + "-sds.pdf\"><span class=\"elementor-icon-list-icon\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-triangle\"></i></span><span class=\"elementor-icon-list-text\">" + v.doc_name + "</span></a></li>");
		            	});
		            }
		...
		setTimeout(function(){ $("#primary-fn").detach().prependTo("#docs-downloads"); }, 1000);
