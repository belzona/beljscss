		...
		$.getJSON( api_url + prod, function(data) {
			...
			$.each(data, function(key,val){
				if(val.iso_code == iso && val.formulation_number){
					...
					if(val.IF){
		            	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + val.IF + "\">Instructions for Use</a></li>");
		            }
		            if(val.PSS){
		            	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + val.PSS + "\">Product Specification Sheet</a></li>");
		            }
		            if(val.CR){
		            	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + val.CR + "\">Chemical Resistant Chart</a></li>");
		            }
		            if(val.SD){
		            	$.each(val.SD, function(k,v){
		                	docs.push("<li class=\"elementor-icon-list-item\"><a href=\"" + v.doc_url + "\">" + v.doc_name + "</a></li>");
		            	});
		            }
